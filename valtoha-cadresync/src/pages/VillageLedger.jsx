...
// import { ArrowLeft, User, MapPin, ClipboardList, Download, Plus, Trash2, Calendar } from 'lucide-react';
...
//     name: '', groupName: '', phone: '', loanLent: '', principal: '', interestPercent: '', balance: ''
//   });
...
//     if (userRole === 'admin') {
//       setAuthorized(true);
//     } else if (activeCadre && activeCadre.villages?.some(v => v.trim().toLowerCase() === decodedVillageName.trim().toLowerCase())) {
//       setAuthorized(true);
//     } else {
//       alert("🚫 Access Denied!");
//       navigate('/login');
...
//         .single();
//       if (data) setCadreInfo(data);
//       else setCadreInfo({ name: 'Not Assigned', phone: 'N/A' });
//     };
//     fetchCadre();
...
//   useEffect(() => {
//     fetchLedgerEntries();
//   }, [authorized, decodedVillageName, selectedYear, selectedMonth]);

//   const handleAddRow = async (e) => {
//     e.preventDefault();
//     if (!newRow.name) return alert("Citizen parameters mismatch.");
//     const activeCadre = JSON.parse(sessionStorage.getItem('active_cadre'));
...
//       installment: (Number(newRow.principal) || 0) + ((Number(newRow.loanLent) || 0) * ((Number(newRow.interestPercent) || 0) / 100)),
//       balance: Number(newRow.balance) || 0,
//       created_by: activeCadre?.cadreId || 'Admin/System',
//       created_at: new Date().toISOString() 
//     };

//     const { error } = await supabase.from('ledger_entries').insert([rowPayload]);
//     if (error) alert("Cloud Sync Failure: " + error.message);
//     else {
//       setNewRow({ name: '', groupName: '', phone: '', loanLent: '', principal: '', interestPercent: '', balance: '' });
//       await fetchLedgerEntries();
//     }
//   };

//   const deleteRow = async (id) => {
//     if (confirm("Permanently wipe out this record line?")) {
//       const { error } = await supabase.from('ledger_entries').delete().eq('id', id);
//       if (!error) await fetchLedgerEntries();
//     }
//   };
...
//         <div>
//           <h3 className="text-[9px] uppercase font-bold text-slate-400">Assigned Cadre</h3>
//           <p className="text-sm font-black text-amber-400">{cadreInfo.name}</p>
//         </div>
//         <div className="text-right">
//           <h3 className="text-[9px] uppercase font-bold text-slate-400">Contact Number</h3>
//           <p className="text-sm font-mono font-bold">{cadreInfo.phone}</p>
//         </div>
//       </div>
...
//         <div className="flex items-center gap-2 border-r border-slate-100 pr-2 w-full md:w-auto">
//           <Calendar size={14} className="text-slate-400 shrink-0" />
//           <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-slate-50 border border-slate-200 text-slate-800 text-xs font-black px-3 py-1.5 rounded-lg focus:outline-none cursor-pointer">
//             {DYNAMIC_YEARS.map(yr => <option key={yr} value={yr}>Financial Year {yr}</option>)}
//           </select>
//         </div>
//         <div className="flex items-center gap-1 overflow-x-auto w-full no-scrollbar">
//           {MONTHS_ARRAY.map(mth => (
//             <button key={mth} onClick={() => setSelectedMonth(mth)} className={`px-3 py-1.5 text-xs font-bold rounded-lg border ${selectedMonth === mth ? 'bg-amber-500 text-white' : 'bg-white border-slate-100 text-slate-500'}`}>
//               {mth}
//             </button>
//           ))}
//         </div>
...
//                 <th className="py-3 px-4">Citizen Identity & SHG Link</th>
//                 <th className="py-3 px-4">Phone</th>
//                 <th className="py-3 px-4">Loan Lent</th>
//                 <th className="py-3 px-4">Principal Remitted</th>
...
//               {fetching ? (
//                 <tr><td colSpan="9" className="p-10 text-center text-slate-400 font-medium text-xs animate-pulse">Streaming transactional blocks...</td></tr>
//               ) : currentMonthEntries.map((row) => (
//                 <tr key={row.id} className="transition-colors border-l-4 bg-white hover:bg-slate-50/30 border-l-transparent">
//                   <td className="py-3 px-4"><p className="font-black text-slate-800">{row.citizen_name}</p><p className="text-[10px] text-slate-400 font-medium italic">{row.group_name}</p></td>
//                   <td className="py-3 px-4 font-mono text-slate-400 text-[11px]">{row.phone}</td>
//                   <td className="py-3 px-4 text-blue-700 font-mono">₹{row.loan_lent?.toLocaleString('en-IN')}</td>
//                   <td className="py-3 px-4 font-mono">₹{row.principal?.toLocaleString('en-IN')}</td>
//                   <td className="py-3 px-4 text-amber-600 font-mono">{row.interest_percent}%</td>
//                   <td className="py-3 px-4 font-mono font-black text-emerald-700 text-sm">₹{row.installment?.toLocaleString('en-IN')}</td>
//                   <td className="py-3 px-4 text-slate-900 font-mono font-black bg-slate-50/20">₹{row.balance?.toLocaleString('en-IN')}</td>
//                   <td className="py-3 px-4 font-mono text-[10px] text-slate-500">{row.created_at ? new Date(row.created_at).toLocaleDateString() : 'N/A'}</td>
//                   <td className="py-3 px-4 text-center"><button onClick={() => deleteRow(row.id)} className="text-slate-300 hover:text-red-500 transition-colors cursor-pointer"><Trash2 size={13} /></button></td>
//                 </tr>
//               ))}
//             </tbody>
//           </table>
...
//             <input type="text" required placeholder="Citizen Full Name" value={newRow.name} onChange={(e) => setNewRow({...newRow, name: e.target.value})} className="w-full px-3 py-1.5 border border-slate-200 text-xs rounded-lg font-semibold" />
//             <input type="text" placeholder="SHG Group (Optional)" value={newRow.groupName} onChange={(e) => setNewRow({...newRow, groupName: e.target.value})} className="w-full px-3 py-1.5 border border-slate-200 text-xs rounded-lg" />
//             <input type="text" placeholder="Contact Mobile (Optional)" value={newRow.phone} onChange={(e) => setNewRow({...newRow, phone: e.target.value})} className="w-full px-3 py-1.5 border border-slate-200 text-xs rounded-lg font-mono" />
//           </div>
//           <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 items-end">
//             <input type="number" placeholder="Loan" value={newRow.loanLent} onChange={(e) => setNewRow({...newRow, loanLent: e.target.value})} className="w-full px-2.5 py-1.5 border border-slate-200 text-xs rounded-lg" />
//             <input type="number" placeholder="Principal" value={newRow.principal} onChange={(e) => setNewRow({...newRow, principal: e.target.value})} className="w-full px-2.5 py-1.5 border border-slate-200 text-xs rounded-lg" />
//             <input type="number" placeholder="Int %" value={newRow.interestPercent} onChange={(e) => setNewRow({...newRow, interestPercent: e.target.value})} className="w-full px-2.5 py-1.5 border border-slate-200 text-xs rounded-lg" />
//             <input type="number" placeholder="Bal" value={newRow.balance} onChange={(e) => setNewRow({...newRow, balance: e.target.value})} className="w-full px-2.5 py-1.5 border border-slate-200 text-xs rounded-lg" />
//             <button type="submit" className="w-full py-1.5 bg-slate-900 text-white text-[10px] font-black uppercase rounded-lg">Commit Row Node</button>
//           </div>
//         </form>
//       </div>
//     </div>
...
//     XLSX.utils.book_append_sheet(wb, ws, `${selectedMonth} Report`);
//     XLSX.writeFile(wb, `${decodedVillageName}_${selectedMonth}_${selectedYear}.xlsx`);
//   };
...
//   const handleAddRow = async (e) => {
//     e.preventDefault();
//     if (!newRow.name) return alert("Citizen name is required.");
//     const activeCadre = JSON.parse(sessionStorage.getItem('active_cadre'));
...
//       timeline: `${selectedYear}-${selectedMonth}`,
//       citizen_name: newRow.name,
//       group_name: newRow.groupName || "Individual",
//       phone: newRow.phone || "N/A",
//       loan_lent: Number(newRow.loanLent) || 0,
//       principal: Number(newRow.principal) || 0,
//       interest_percent: Number(newRow.interestPercent) || 0,
//       installment: (Number(newRow.principal) || 0) + ((Number(newRow.loanLent) || 0) * ((Number(newRow.interestPercent) || 0) / 100)),
//       balance: Number(newRow.balance) || 0,
//       created_by: activeCadre?.cadreId || 'Admin',
//       created_at: new Date().toISOString() 
//     };
...
//         <button onClick={handleDownloadExcel} className="flex items-center gap-1.5 px-4 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-bold uppercase hover:bg-emerald-700 cursor-pointer shadow-sm">
...
//         </div>
//         <div className="text-right">
//           <h3 className="text-[9px] uppercase font-bold text-slate-400">Contact</h3>
//           <p className="text-sm font-mono font-bold">{cadreInfo.phone}</p>
//         </div>
//       </div>

//       <div className="max-w-7xl mx-auto mb-6 bg-white border border-slate-100 rounded-xl p-3 shadow-xs flex flex-col md:flex-row items-center gap-4">
//         <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-slate-50 border border-slate-200 text-xs font-black px-3 py-1.5 rounded-lg cursor-pointer">
//           {DYNAMIC_YEARS.map(yr => <option key={yr} value={yr}>Year {yr}</option>)}
//         </select>
...
//             {currentMonthEntries.map((row) => (
//               <tr key={row.id}>
//                 <td className="py-3 px-4 font-bold text-slate-800">{row.citizen_name}</td>
...
//                 <td className="py-3 px-4 font-bold text-emerald-700">₹{row.installment}</td>
//                 <td className="py-3 px-4">{row.interest_percent}%</td>
//                 <td className="py-3 px-4 font-black">₹{row.balance}</td>
//                 <td className="py-3 px-4"><button onClick={() => deleteRow(row.id)}><Trash2 size={13} className="text-slate-400 hover:text-red-500"/></button></td>
//               </tr>
//             ))}
//           </tbody>
//         </table>
...
const VillageLedger = () => {
 ...
 const handleDownloadExcel = () => {
  ...
  // const handleDownloadExcel = () => {
  //   const ws = XLSX.utils.json_to_sheet(currentMonthEntries.map(item => ({
  //     "Name": item.citizen_name, "Contact": item.phone, "Loan": item.loan_lent, "Date": item.created_at, "Installment": item.installment, "Interest": item.interest_percent, "Balance": item.balance
  //   })));
  //   const wb = XLSX.utils.book_new();
  //   XLSX.utils.book_append_sheet(wb, ws, "Sheet1");
  //   XLSX.writeFile(wb, `${decodedVillageName}_Report.xlsx`);
  ...
  //   e.preventDefault();
  //   const rowPayload = {
  //     village_name: decodedVillageName, timeline: `${selectedYear}-${selectedMonth}`,
  //     citizen_name: newRow.name, group_name: newRow.groupName || "Individual Ledger",
  //     phone: newRow.phone || "N/A", loan_lent: Number(newRow.loanLent) || 0,
  //     principal: Number(newRow.principal) || 0, interest_percent: Number(newRow.interestPercent) || 0,
  //     installment: (Number(newRow.principal) || 0) + ((Number(newRow.loanLent) || 0) * ((Number(newRow.interestPercent) || 0) / 100)),
  //     balance: Number(newRow.balance) || 0, created_at: newRow.date
  //   };
  //   const { error } = await supabase.from('ledger_entries').insert([rowPayload]);
  //   if (!error) {
  //     setNewRow({ name: '', groupName: '', phone: '', loanLent: '', principal: '', interestPercent: '', balance: '', date: new Date().toISOString().split('T')[0] });
  //     fetchLedgerEntries();
  //   }
  // };
